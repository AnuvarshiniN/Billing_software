...
void Customer::on_pBSave_clicked()
{
    ...
    if (ui->lEMobileNo->text().simplified().isEmpty()) {
        ...
        ui->lEMobileNo->setFocus();
        return;
    }

    QRegularExpression regexp("^[0-9]{10}$");
    if (regexp.match(ui->lEMobileNo->text()).hasMatch() == false) {
        QMessageBox::information(this, "Missing", "Invalid Mobileno");
        ui->lEMobileNo->setFocus();
        return;
    }
